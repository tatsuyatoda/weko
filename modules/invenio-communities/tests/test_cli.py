...
def test_request(script_info,db_records,communities):
    patch("invenio_records.api.before_record_update.send")
    patch("invenio_records.api.after_record_update.send")
    patch("invenio_communities.models.inclusion_request_created.send")
...
def test_remove(script_info,communities,db_records):
    patch("invenio_records.api.before_record_update.send")
    patch("invenio_records.api.after_record_update.send")
    patch("invenio_communities.models.inclusion_request_created.send")
    ...
    mock_index = patch("invenio_communities.cli.RecordIndexer.delete_by_id")
    result = runner.invoke(
        remove,
        [comm.id,record_id],
        obj=script_info
